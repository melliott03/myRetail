new Vue({
    ...
    mounted () {
        ...
        .then(response => {
            ...
            this.alternateImages = response.data.CatalogEntryView[0].Images[0].AlternateImages;
            ...
            console.log('catalogEntryRe::', catalogEntryRe);

            for (let index = 0; index < arrayItm.length; index++) {
                console.log('inside for loop response.data, index::', index );

                console.log('catalogEntryRe::', catalogEntryRe);
                console.log('arrayItm[index]::', arrayItm[index]);

                this.catalogEntryRe.push( arrayItm[index] ); 
            }
            console.log('after for loop response.data, this.catalogEntryRe::', this.catalogEntryRe );

            // this.catalogEntryRe = this.catalogEntry.CatalogEntryView[0].Images[0].AlternateImages[0];
            // [].concat();
        })
    },
    methods: {
