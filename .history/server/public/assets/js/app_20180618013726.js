new Vue({
    ...
    mounted () {
        ...
        .then(response => {
            ...
            let arrayItm = response.data.CatalogEntryView[0].Images[0].AlternateImages[0];
            console.log('before for loop response.data,response.data.CatalogEntryView[0].Images[0].AlternateImages[0]::', response.data.CatalogEntryView[0].Images[0].AlternateImages[0]);

            for (let index = 0; index < arrayItm.length; index++) {
                console.log('inside for loop response.data::' );

                console.log('altImages::', altImages);
                console.log('arrayItm[index]::', arrayItm[index]);

                this.altImages.push( array[index] ); 
            }
            console.log('after for loop response.data::' );

            // this.altImages = this.catalogEntry.CatalogEntryView[0].Images[0].AlternateImages[0];
            // [].concat();
        })
    },
    methods: {
